...
7. 已新增 Electron UI E2E 入口：`pnpm run test:e2e:ui`，使用 Playwright 驱动真实桌面应用，校验教程首页、侧边栏导航与工作区上下文在关键页面间保持一致。
8. 当前发布脚本会显式使用 Windows 目标打包（`electron-builder --win`），避免在 Linux/WSL 环境下默认产出 AppImage / Snap 资产。
