...
> 当前应用已经把“发布”和“备份”收敛到 **统一发布与备份** 页面。
> 如果你登录的是 GitHub 账号，用户站点会直接把发布仓库名固定为 `用户名.github.io`，备份仓库固定为 `BFE`。
...
3. 用户站点会直接使用 `用户名.github.io` 作为发布仓库名，不需要在应用里再手动发明一个名字。

## 2. 再准备备份仓库

1. 在同一个 GitHub 账号下再创建一个仓库。
2. 备份仓库固定使用：`BFE`。
3. 这个仓库用于恢复/迁移你的完整工作区，不是公开博客地址。
...
1. 打开 发布与备份 页面。
2. 选择当前工程。
3. 先登录 GitHub，让应用自动带入 GitHub 用户名。
4. 如果你发布的是用户站点，应用会直接把发布仓库固定成 `用户名.github.io`。
5. 如果你发布的是 project pages，再手动填写 project 仓库名。
6. 选择备份目录。
7. 点击 一键发布。
...
5. 只能恢复备份仓库，找不到发布仓库。
处理：这是允许的。`BFE` 备份仓库才是恢复入口；发布仓库缺失时，先把工程恢复到本地再重新配置发布。
