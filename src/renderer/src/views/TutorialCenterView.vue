...
const tutorialSections = [
    ...
    },
    {
        ...
        summary: '面向小白的 OAuth 配置与登录步骤，避免把 Client ID、Token 和用户名混淆。',
        steps: [
    ...
    },
    {
        key: 'create-blog',
        title: '02. 创建博客工程',
        summary: '解释每个输入框该填什么、为什么这样填，以及慢速阶段如何判断是否正常。',
        steps: [
    ...
    },
    {
        key: 'theme-images',
        title: '03. 主题配置与图片',
        summary: '不使用图床。直接把本地图片转存到工程内图片目录（默认 img），并自动回填可用 URL。',
        steps: [
            '进入“主题配置”，先选择工程与主题。',
            '在“背景与图标”里填写本地背景图路径。',
    ...
    },
    {
        ...
        title: '04. 评论与浏览量统计',
        summary: '默认推荐“零额外操作”的浏览量方案，并提供 Giscus/Umami/GA 的进阶配置。',
        steps: [
            '浏览量统计默认启用“不蒜子”，一般不需要额外注册。',
            '如果你需要更详细数据，再补 Umami 或 GA。',
            '配置 Giscus 前先开启仓库 Discussions，再去 giscus.app 复制参数。',
            '将 repo、repoId、category、categoryId 填入应用后保存。'
        ],
        notes: [
            '广告拦截插件会影响统计脚本加载，测试时可先临时关闭。',
            'Giscus 的 repo 必须是 owner/repo 格式。'
        ]
    },
    {
        key: 'content',
...
const activeSection = computed(() => {
    return tutorialSections.find((item) => item.key === activeSectionKey.value) || tutorialSections[0];
});
</script>
...
<template>
    <section class="panel tutorial-panel tutorial-layout">
        <aside class="tutorial-directory">
            ...
            <button v-for="section in tutorialSections" :key="section.key"
